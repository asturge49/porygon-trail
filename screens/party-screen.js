                    ...
                    ${state.party.map((p, i) => `
                        <div class="party-slot ${p.status === 'fainted' ? 'status-fainted' : ''}">
                            ...
                            <div class="party-info">
                                ...
                                <div>Stars: ${'★'.repeat(p.battleStars || 0)}${'☆'.repeat(3 - (p.battleStars || 0))} <span style="font-size:5px;">(${p.battleWins || 0} wins)${PT.Engine.GameState.isFinalEvolution ? (PT.Engine.GameState.isFinalEvolution(p, state) ? '' : ' [needs final evo]') : ''}</span></div>
